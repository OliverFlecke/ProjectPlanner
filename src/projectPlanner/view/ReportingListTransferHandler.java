...
public class ReportingListTransferHandler extends TransferHandler {
	...
	public ReportingListTransferHandler(){
		...
		try {
			...
			System.out.println("ReportingListTransferHandler: unable to create date flavor");
		}
	...
	public boolean importData(JComponent c, Transferable t){
		...
		try {
			...
			System.out.println("importData: I/= exception");
			return false;
		}
		...
		if(target.getName().equals("right") && target != source) {
			...
			int retVal = JOptionPane.showConfirmDialog(target, message, "Confirm", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
			System.out.println("retVal =" + retVal);
			if (retVal == JOptionPane.NO_OPTION || retVal == JOptionPane.CLOSED_OPTION);
			return false;
		}
		
		//At this point we use the sa´me code t oretrive the date locally or serially.
		...
		//For example, if the user is moving item #4, #5, #6 and #7 and
	...
	protected void exportDone(JComponent c, Transferable data, int action){
		if ((action == MOVE) && (indices != null)){
			...
			//need to adjust the indices accordingly since those after
			//the insertion point moved.
			if (addCount > 0) {
	...
	protected Transferable createTransferable(JComponent c) {
		if ( c instanceof JList) {
			...
			return new ReportingListTransferable(alist, localArrayListFlavor, serialArrayListFlavor);
		}
		return null;
	}
